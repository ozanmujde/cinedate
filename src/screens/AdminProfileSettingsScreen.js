...
const AdminProfileSettingsScreen = ({route: {params}}) => {
  ...
  const [about, setAbout] = useState(params.user.About);
  const [bio, setBio] = useState(params.user.Bio);
  const [interests, setInterests] = useState(params.user.Interests);
  const [location, setLocation] = useState(params.user.Location);
  const [username, setUsername] = useState(params.user.Username);

  const generator = new AvatarGenerator();
  //generator.generateRandomAvatar()
  const [profilePhoto, setProfilePhoto] = useState(params.user.ProfilePhoto);
